import { Utils } from "../../utils.b8ac3bbf.js";
import { LoadingBar } from "../../loadingbar.1f15acf7.js";
import { Charts } from "../../charts.3f0590e4.js";
import { WordCloud } from "../../wordcloud.cfb2abe4.js";
...
if (charts.length > 0) {
    ...
    for (const el of charts) {
        ...
        switch (el.dataset.chart) {
            case "ratio-bar":
                Charts.createRatioBarChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, normalized);
                break;
            case "bar":
            ...
            case "column":
                ...
                break;
            case "pop-pyramid":
                Charts.createPopPyramidChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, el.dataset.bounds?.parseFloat() ?? 15, horizontal);
                break;
            case "pie":
                Charts.createPieChart(chartId, master[dataKey], title, colors, height);
                break;
            case "boxplot":
                Charts.createBoxPlot(chartId, master[dataKey], title, subtitle, height, el.dataset.bounds?.parseFloat() ?? undefined, vertical, false, upperColor, lowerColor, heightInches);
                break;
            case "change-boxplot":
                Charts.createBoxPlot(chartId, master[dataKey], title, subtitle, height, el.dataset.bounds?.parseFloat() ?? 5, vertical, true, upperColor, lowerColor);
                break;
            case "scatter":
                Charts.createScatterPlot(chartId, master[dataKey], title, subtitle, hideSeries, colors, height);
                break;
            case "heatmap":
                Charts.createHeatmap(chartId, master[dataKey], title, subtitle, color, height);
                break;
        }
