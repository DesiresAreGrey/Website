import { Utils } from "../../utils/utils.b8ac3bbf.js";
import { LoadingBar } from "../../utils/loadingbar.1f15acf7.js";
import { Apex } from "../../charts/apex.a516fcf0.js";
import { WordCloud } from "../../charts/wordcloud.cfb2abe4.js";
...
if (charts.length > 0) {
    ...
    for (const el of charts) {
        ...
        switch (el.dataset.chart) {
            case "ratio-bar":
                Apex.createRatioBarChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, normalized);
                break;
            case "bar":
                Apex.createBarChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, true);
                break;
            case "column":
                Apex.createBarChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, false);
                break;
            case "pop-pyramid":
                Apex.createPopPyramidChart(chartId, master[dataKey], title, subtitle, hideSeries, colors, height, el.dataset.bounds?.parseFloat() ?? 15, horizontal);
                break;
            case "pie":
                Apex.createPieChart(chartId, master[dataKey], title, colors, height);
                break;
            case "boxplot":
                Apex.createBoxPlot(chartId, master[dataKey], title, subtitle, height, el.dataset.bounds?.parseFloat() ?? undefined, vertical, false, upperColor, lowerColor, heightInches);
                break;
            case "change-boxplot":
                Apex.createBoxPlot(chartId, master[dataKey], title, subtitle, height, el.dataset.bounds?.parseFloat() ?? 5, vertical, true, upperColor, lowerColor);
                break;
            case "scatter":
                Apex.createScatterPlot(chartId, master[dataKey], title, subtitle, hideSeries, colors, height);
                break;
            case "heatmap":
                Apex.createHeatmap(chartId, master[dataKey], title, subtitle, color, height);
                break;
        }
