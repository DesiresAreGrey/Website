...
const parseVersions = (input) => input
    .split(/(?:,| |\n)/)
    .map(v => v.trim())
    .filter(v => v.length > 0 && !isNaN(Utils.compareVersions(v, v)))
    .sort(Utils.compareVersions);
const versionsChanged = (input, currentVersions) => parseVersions(input).join(", ") === currentVersions?.join(", ");
...
async function loadOverallCharts(loadStart = 0, loadEnd = 1) {
    const input = $id("overall-versions-input");
    const updateButton = $id("overall-versions-update");
    updateButton.versions ?? (updateButton.versions = startingVersions);
...
async function loadModrinthCharts(loadStart = 0, loadEnd = 1) {
    const input = $id("modrinth-versions-input");
    const updateButton = $id("modrinth-versions-update");
    updateButton.versions ?? (updateButton.versions = startingVersions);
    input.value = updateButton.versions.join(", ");
    input.addEventListener("keydown", enterToUpdate);
    input.addEventListener("input", () => toggleUpdateButton(updateButton, input.value));
...
async function loadCurseforgeCharts(loadStart = 0, loadEnd = 1) {
    const input = $id("curseforge-versions-input");
    const updateButton = $id("curseforge-versions-update");
    updateButton.versions ?? (updateButton.versions = startingVersions);
    input.value = updateButton.versions.join(", ");
    input.addEventListener("keydown", enterToUpdate);
    input.addEventListener("input", () => toggleUpdateButton(updateButton, input.value));
...
function toggleUpdateButton(button, input) {
    console.log(parseVersions(input));
    button.classList.toggle("disabled", versionsChanged(input, button.versions));
}
function showApiRequests(containerId, requests, apiName) {
    const inputWrapper = $id(containerId)?.$(".input-wrapper");
    if (inputWrapper) {
